...
var keyGG: any = process.env.GOOGLE_LOG_KEY;

var keyCC: any = process.env.GOOGLE_CLIENT_ID;
...
app.post("/feeds_Topic", async (req: Request, res: Response) => {
  const { values } = req.body;

  if (values.postPageLimit == 0) {
    try {
      const chronologicaldata = await execPoolQuery(postTopic, [
        values.id,
        values.id,
        values.topic,
      ]);
...
app.post("/feeds_chronologicalAll", async (req: Request, res: Response) => {
  const { values } = req.body;

  if (values.postPageLimit == 0) {
    try {
      const chronologicaldata = await execPoolQuery(postsxAll, [
        values.id,
        values.id2,
      ]);
    ...
    try {
      const chronologicaldata = await execPoolQuery(postsxAllmore, [
        values.id,
        values.id2,
        values.postPageLimit,
      ]);
...
async function generateVideoFromImage(params: any) {
  const apiHost = "https://api.stability.ai";
  const apiKey = APP_STATE_STABLE_KEY;
  ...
  const response = await fetch(`${apiHost}/v2beta/image-to-video`, {
    method: "POST",
    headers: {
      Authorization: `Bearer ${apiKey}`,
      ...form.getHeaders(),
      Accept: "video/*", // Expect video data
    },
    body: form,
  });
  ...
  while (!videoReady) {
    ...
    videoResponse = await fetch(
      `${apiHost}/v2beta/image-to-video/result/${id}`,
      {
        method: "GET",
        headers: {
          Authorization: `Bearer ${apiKey}`,
    ...
    if (videoResponse.status === 200) {
      ...
      console.log(
        "Video generation in progress, polling again in 10 seconds..."
      );
    }
  }
...
app.post("/generatevideo", async (req, res) => {
  ...
  try {
    const videoBuffer = await generateVideoFromImage(params);
...
app.post("/loge", async (req: Request, res: Response) => {
  ...
  try {
    const logindata = await execPoolQuery(loginEmail, [values.email]);
    ...
    const days30inseconds = 60 * 60 * 24 * 30 * 1000;
    const CurrentTimePlusSecs = new Date(
      new Date().getTime() + days30inseconds
    );
...
////
////
app.post("/registrationGoogle", async (req: Request, res: Response) => {
  const { values } = req.body;
